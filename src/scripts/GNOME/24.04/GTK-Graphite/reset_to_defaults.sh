...
# Begin GTK Graphite Theme Removal
...
if check_internet; then
    ...
    log_message "INFO" "Removing GTK Graphite Theme"
    printc "YELLOW" "-> Removing GTK Graphite Theme..."
    rm -rf "${HOME}/.themes/Graphite*" || handle_error "Failed to remove GTK Graphite Theme"

    log_message "INFO" "Removing GTK Graphite Icons"
    printc "YELLOW" "-> Removing GTK Graphite Icons..."
    rm -rf "${HOME}/.local/share/icons/Tela-*" || handle_error "Failed to remove GTK Graphite Icons"
    ...
    if whiptail --title "GTK Graphite Theme Removal" --yesno "GTK Graphite Theme has been removed successfully.\nDo you want to sign out to fully reset ?" 10 60; then
...
# End GTK Graphite Theme Removal